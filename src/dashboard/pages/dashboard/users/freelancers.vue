...
const routes = [
  ...
  {
    path: 'dashboard-users-freelancers',
    breadcrumbName: 'Freelances',
  },
]
...
const columns: TableColumnsType = [
  ...
  },
  {
    ...
    customFilterDropdown: true,
    onFilter: (value, record) =>
      record.firstName.toString().toLowerCase().includes(value.toLowerCase()),
    onFilterDropdownVisibleChange: (visible) => {
      if (visible) {
        setTimeout(() => {
          searchInput.value.focus()
...
const getUsers = async() => {
  const { data, error } = await apiServices('/freelancer/all').json()
...
const updateOrCreateUser = async(dataUser) => {
  if (dataUser._id) {
    ...
    delete dataUser._id
    const { data: updateUser, error } = await apiServices(`/freelancer/update-freelance/${userId}`).patch(dataUser).json()
...
const deleteUser = async(userId) => {
  const { data: deleteUser, error } = await apiServices(`/freelancer/delete-freelancer/${userId}`).patch().json()
  deleteUser.value && !error.value && (message.success(deleteUser.value?.message))
  getUsers()
}
const blockOrUnblockUser = async(userId, validated) => {
  if (validated) {
    const { data: blockUser, error } = await apiServices(`/freelancer/block-freelancers/${userId}`).patch().json()
    blockUser.value && !error.value && (message.success(blockUser.value?.message))
    getUsers()
  }
  else {
    const { data: blockUser, error } = await apiServices(`/freelancer/unblock-freelancers/${userId}`).patch().json()
    blockUser.value && !error.value && (message.success(blockUser.value?.message))
    getUsers()
  }
}
const validateOrUnvalidateUser = async(userId, doc_validated) => {
  if (doc_validated) {
    const { data: docUser, error } = await apiServices(`/freelancer/documents-unvalidated/${userId}`).patch().json()
    docUser.value && !error.value && (message.success(docUser.value?.message))
    getUsers()
  }
  else {
    const { data: docUser, error } = await apiServices(`/freelancer/documents-validated/${userId}`).patch().json()
    docUser.value && !error.value && (message.success(docUser.value?.message))
    getUsers()
  }
}
...
<template>
  <div>
    <a-page-header
      ...
      title="Freelances" :breadcrumb="{ routes }" sub-title="Liste des freelances"
    >
      <template #tags>
        <a-tag color="blue">
          Running
...
meta:
  layout: admin
  siderMenu:
    name: dashboard.users.freelancers
    index: 1
    parentName: dashboard.users.parent
    link: dashboard-users-freelancers
