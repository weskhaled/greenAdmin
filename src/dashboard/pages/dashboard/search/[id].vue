...
const getFormData = async() => {
  ...
  if (props.id) {
    const { data: dataProfiles, error: errorProfiles } = await apiServices(`/missions/profiles/${props.id}`).json()
    if (dataProfiles.value && !errorProfiles.value) {
      freelancers.value = dataProfiles.value.data.freelancers
      freelancersNeeded.value = dataProfiles.value.data.freelancersNeeded
      freelancersAppreciated.value = dataProfiles.value.data.freelancersAppreciated
      freelancersLoading.value = false
    }
    else {
      message.error(errorProfiles.value)
    }
    const { data: dataProfilesAgence, error: errorProfilesAgence } = await apiServices(`/missions/profiles-agences/${props.id}`).post({ minPrice: 50, maxPrice: 99999 }).json()
    if (dataProfilesAgence.value && !errorProfilesAgence.value) {
      agences.value = dataProfilesAgence.value.data.agences
      offers.value = dataProfilesAgence.value.data.offers
      offersUnfiltred.value = dataProfilesAgence.value.data.offers
      agencesLoading.value = false
    }
    else {
      message.error(errorProfilesAgence.value)
    }
  }
  else {
    freelancersLoading.value = false
