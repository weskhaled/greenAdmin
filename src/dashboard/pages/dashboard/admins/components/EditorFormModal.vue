...
const emit = defineEmits(['updateOrCreateEditor'])
...
const onSubmit = () => {
  validate()
    .then(() => {
      emit('updateOrCreateEditor', { ...toRaw(modelRef), _id: props.user?._id || undefined, email: props.user?.email || modelRef.email })
    })
    .catch((err) => {
