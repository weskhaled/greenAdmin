...
const routes = [
  ...
  {
    path: 'dashboard-notifications-agence',
    breadcrumbName: 'notifications agence',
  },
]
...
const getNotifs = async() => {
  const { data, error } = await apiServices('/notification-agence/unchecked/').json()
...
const validateOrUnvalidateNotif = async(notification) => {
  ...
  if (!notification.checked) {
    const { data: docAdmin, error } = await apiServices(`/notification-agence/check/${notification._id}`).patch(params).json()
    docAdmin.value && !error.value && (message.success(docAdmin.value?.message))
    getNotifs()
  }
  else {
    const { data: docAdmin, error } = await apiServices(`/notification-agence/uncheck/${notification._id}`).patch(params).json()
    docAdmin.value && !error.value && (message.success(docAdmin.value?.message))
    getNotifs()
  }
}
const filterDate = async() => {
  ...
  const { data: datePeriod, error } = await apiServices('/notification-agence/period').post(params).json()
...
<template>
  <div>
    <a-page-header
      ...
      title="Notifications solution Innovante" :breadcrumb="{ routes }" sub-title="Liste des notifications"
    >
      <template #tags>
        <a-tag color="blue">
          Running
  ...
  >
    <div>
      ...
      <label v-if="formStateChanges.changes.domain"><b>Domaine : </b> {{ formStateChanges.changes.
        domain }} <br></label>
      <label v-if="formStateChanges.changes.price"><b>Prix : </b> {{ formStateChanges.changes.
        price }} <br></label>
      <label v-if="formStateChanges.changes.show_price"><b>Affichage du prix : </b> {{ formStateChanges.changes.
        show_price ? 'Oui' : 'Non' }} <br></label>
      <label v-if="formStateChanges.changes.description"><b>Description : </b> {{ formStateChanges.changes.
      ...
      <label v-if="formStateChanges.changes.filename && formStateChanges.changes.idAgence"><b><a-button size="small" class="mr-1 inline-block" type="link" warn>
        <span class="i-carbon-view inline-block" @click.prevent="$router.push(`/dashboard/users/agence/${formStateChanges.changes.idAgence}`)" />
      </a-button></b><br></label>
      ...
      <label v-if="formStateChanges.changes.state"><b>Etat :</b>
        ...
        <div v-else-if="formStateChanges.changes.state === 'offer_added'"><a-tag
          class="text-xs ml-2 leading-5"
          color="#05f"
        >
          Offre ajoutée
        ...
        <div v-else-if="formStateChanges.changes.state === 'offer_updated'"><a-tag
          class="text-xs ml-2 leading-5"
          color="#05f"
        >
          Offre modifiée
        ...
        <div v-else-if="formStateChanges.changes.state === 'offer_upload'"><a-tag
          class="text-xs ml-2 leading-5"
          color="#05f"
        >
          Document de l'Offre téléchargé
        </a-tag>
        </div>
        <div v-else-if="formStateChanges.changes.state === 'offer_upload'"><a-tag
          class="text-xs ml-2 leading-5"
          color="#05f"
        >
          Document d'une Offre téléchargé
        </a-tag>
        </div>
        <div v-else="formStateChanges.changes.state === 'offer_deleted'"><a-tag
          class="text-xs ml-2 leading-5"
          color="#05f"
        >
          Offre effacée
...
meta:
  layout: admin
  siderMenu:
    name: dashboard.notifications.agence
