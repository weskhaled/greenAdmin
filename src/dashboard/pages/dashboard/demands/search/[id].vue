...
const sendUpdateDemand = async() => {
  const data = {
    search: {
      freelancers: freelancers.value,
      freelancersNeeded: freelancersNeeded.value,
      freelancersAppreciated: freelancersAppreciated.value,
    },
  }
  const { data: dataSendDemand, error: errorSendDemand } = await apiServices(`/demands/${props.id}/`).patch(data).json()
  if (dataSendDemand.value && !errorSendDemand.value)
    message.info(dataSendDemand.value.message)
  else
    message.error(errorSendDemand.value)
}
              ...
              <a-tab-pane key="1" tab="Recherche Freelance">
                <div class>
                  <a-card title="Freelance correspondants" :bordered="false" class="rounded-sm">
                    ...
                    <div v-else>
                      <h3>Profils Correspondants</h3>  <a-button type="primary" block :loading="profileEntrepriseLoading" @click="sendUpdateDemand()">
                        envoyer le résultat de la recherche
                      </a-button>
